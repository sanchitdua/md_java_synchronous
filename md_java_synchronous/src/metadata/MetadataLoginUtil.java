		...
		try {
			...
			while(!isDone){
				...
				try{
					...
					final LoginResult loginResult = loginToSalesforce(user, pass, authEndPoint); // logging in using enterprise wsdl
					mc = createMetadataConnection(loginResult); // creating metadata connection
					isDone = true;
